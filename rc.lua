...
require("volume")
-- https://github.com/koenwtje/awesome-batteryInfo/blob/master/battery.lua
require("battery")
...
-- beautiful.init("/usr/share/awesome/themes/solarized/theme.lua")

-- Initialize theme from local (user) directory
-- https://github.com/pw4ever/awesome-wm-config/blob/master/rc.lua
...
-- {{{ Tags
-- http://awesome.naquadah.org/wiki/My_first_awesome#Personalize_your_tags
tags = {
  names = { "term", "web", "docs", "comm", "media", 6, 7, "shell", 9 },
  layout = { layouts[1], layouts[7], layouts[1], layouts[7], layouts[1], layouts[1], layouts[1], layouts[7], layouts[1] }
}
...
mytextclock = awful.widget.textclock({ align = "right" }, " %a %Y-%m-%d · %H:%M ")
...
-- Show battery level immediately (before the hook runs for the first time).
